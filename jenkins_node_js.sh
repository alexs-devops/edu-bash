...
##########################################################################################
###
### Script to deploy nodejs. Accepts parameters:
...
###  $4 -> Application name from values.yaml (nodejs|nodejsMC|nodejs)
###
##########################################################################################
...
CONF_DIR="/usr/opt/app/Ecom_V9/nodejs"
WRK_DIR="/usr/opt/app/Ecom_V9/source/nodejs"
...
case ${APP_VALUES} in 
   ...
   *)
      echo "[ERROR]: ${APP_VALUES} incorrect App name. Should be nodejs or nodejsMC ro nodejs."
   ;;
...
case ${ENV} in
   ...
   TST1|DEV1|TST2|DEV2|DEV3|TST3|TST9|DEV9)
      ...
      GC_IMAGE="us.gcr.io//commerce/nodejs"

      if [[ "${suffix}" = "no" ]];
      then
         APP_NAME="nodejs"
         GC_WRKLOAD="${GC_PREFIX}${env_num}${env_purp}nodejs"
      else
         APP_NAME="nodejs-${suffix}"
         GC_WRKLOAD="${GC_PREFIX}${env_num}${env_purp}nodejs-${suffix}"
      fi
   ...
   STG|PRD)
      ...
      then
         APP_NAME="nodejs"
         GC_WRKLOAD="ecomprod${env_purp}nodejs"
      else
         APP_NAME="nodejs-${suffix}"
         GC_WRKLOAD="ecomprod${env_purp}nodejs-${suffix}"
      fi
...
. /usr/opt/app/Ecom_V9/scripts/jenkins_deployment_count.sh $1 nodejs build
